...
SHIGURE_DEBUG_CACHE_MAX_BYTES = int(
    os.environ.get("SHIGURE_DEBUG_CACHE_MAX_BYTES", str(10 * 1024 * 1024 * 1024))
)
...
SHIGURE_LIFECYCLE_NO_MOVE_DISTANCE_M = float(
    os.environ.get("SHIGURE_LIFECYCLE_NO_MOVE_DISTANCE_M", "0.20")
)
SHIGURE_LIFECYCLE_CONFIRMATION_SECONDS = float(
    os.environ.get("SHIGURE_LIFECYCLE_CONFIRMATION_SECONDS", "3.0")
)
SHIGURE_LIFECYCLE_OBSERVATION_HZ = float(
    os.environ.get("SHIGURE_LIFECYCLE_OBSERVATION_HZ", "5.0")
)
SHIGURE_LIFECYCLE_LOOKBACK_SECONDS = float(
    os.environ.get("SHIGURE_LIFECYCLE_LOOKBACK_SECONDS", "3.0")
)
SHIGURE_LIFECYCLE_DEPTH_CHANGE_M = float(
    os.environ.get("SHIGURE_LIFECYCLE_DEPTH_CHANGE_M", "0.06")
)
SHIGURE_LIFECYCLE_MIN_BACKGROUND_REVEAL_RATIO = float(
    os.environ.get("SHIGURE_LIFECYCLE_MIN_BACKGROUND_REVEAL_RATIO", "0.35")
)
SHIGURE_LIFECYCLE_MAX_FOREGROUND_OCCLUSION_RATIO = float(
    os.environ.get("SHIGURE_LIFECYCLE_MAX_FOREGROUND_OCCLUSION_RATIO", "0.25")
)
SHIGURE_LIFECYCLE_MIN_SOURCE_AREA_RATIO = float(
    os.environ.get("SHIGURE_LIFECYCLE_MIN_SOURCE_AREA_RATIO", "0.65")
)
SHIGURE_LIFECYCLE_MIN_VALID_DEPTH_RATIO = float(
    os.environ.get("SHIGURE_LIFECYCLE_MIN_VALID_DEPTH_RATIO", "0.70")
)
SHIGURE_LIFECYCLE_MAX_PERSON_OVERLAP_RATIO = float(
    os.environ.get("SHIGURE_LIFECYCLE_MAX_PERSON_OVERLAP_RATIO", "0.15")
)
SHIGURE_LIFECYCLE_MIN_POST_EVIDENCE_FRAMES = int(
    os.environ.get("SHIGURE_LIFECYCLE_MIN_POST_EVIDENCE_FRAMES", "2")
)
SHIGURE_ID_HANDOFF_GRACE_SECONDS = float(
    os.environ.get("SHIGURE_ID_HANDOFF_GRACE_SECONDS", "60.0")
)
...
SHIGURE_HOLO_SYNC_STABLE_MASK_FRAMES = int(
    os.environ.get("SHIGURE_HOLO_SYNC_STABLE_MASK_FRAMES", "2")
)
...
if (
    ...
    or SHIGURE_LIFECYCLE_CONFIRMATION_SECONDS
    < SHIGURE_LIFECYCLE_SELECTION_WINDOW_SECONDS
    or not 0.0 < SHIGURE_LIFECYCLE_OBSERVATION_HZ <= 30.0
    or not 0.0 < SHIGURE_LIFECYCLE_LOOKBACK_SECONDS <= 60.0
    or SHIGURE_LIFECYCLE_DEPTH_CHANGE_M <= 0.0
    or not 0.0
    ...
    <= 1.0
    or not 0.0 < SHIGURE_LIFECYCLE_MIN_SOURCE_AREA_RATIO <= 1.0
    or not 0.0 < SHIGURE_LIFECYCLE_MIN_VALID_DEPTH_RATIO <= 1.0
    or not 0.0 <= SHIGURE_LIFECYCLE_MAX_PERSON_OVERLAP_RATIO <= 1.0
    or SHIGURE_LIFECYCLE_MIN_POST_EVIDENCE_FRAMES < 1
):
    raise ValueError("invalid Shigure lifecycle/recovery timing settings")
if (
...
if (
    ...
    or SHIGURE_HOLO_SYNC_STABLE_MASK_FRAMES < 2

#!/usr/bin/env python3
"""Continuously cache Shigurei RGB-D images and deduplicated camera parameters."""
...
SERVER_SITE_PACKAGES = Path(
    os.environ.get(
        'SHIGURE_HISTORY_RECORDER_CV_SITE_PACKAGES',
...
def write_camera_info_json(path: Path, state: TopicState) -> None:
    msg = state.message
    write_json(
        path,
        {
...
def write_status(cache: ShigureRgbdCache, payload: dict[str, Any]) -> None:
...
def append_cached_sample(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix='sample_', dir=str(tmp_root)) as tmp_name:
        tmp = Path(tmp_name)
        ...
        write_camera_info_json(camera_info_path, states['camera_info'])

        sample = cache.append_sample(
            stamp=stamp,
            rgb_path=rgb_path,
...
def main() -> int:
    ...
    node = Node('shigure_rgbd_history_recorder')
    cache = ShigureRgbdCache(
        args.cache_root,
        ...
        while _running:
            ...
            if not required_ready(states):
                if now - last_log >= max(1.0, float(args.log_interval)):
                    ...
                    write_status(cache, {'running': True, 'waiting_for': missing, 'topic_counts': {name: state.count for name, state in states.items()}})
                    last_log = now
                continue

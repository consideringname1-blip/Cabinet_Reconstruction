...
def _parse_foundationpose_stdout(stdout: str) -> dict:
    for line in reversed((stdout or "").splitlines()):
        ...
        try:
            ...
            continue
    raise ValueError("FoundationPose worker did not emit a JSON result")


def solve_foundationpose_alignment(
    *,
    task: dict,
    paths: dict[str, Path],
    model_vertices_unity: np.ndarray,
    ...
    rotation = (
        FOUNDATIONPOSE_CV_TO_CANONICAL_RH_BASIS
        @ rotation_cv
        @ MODEL_INPUT_TO_CANONICAL_RH_BASIS.T
    ).astype(np.float32)
    translation = (FOUNDATIONPOSE_CV_TO_CANONICAL_RH_BASIS @ translation_cv.reshape(3, 1)).reshape(3).astype(np.float32)
...
def main(argv: list[str]) -> int:
    json_path, task = load_stage_task(
        ...
        usage=(
            ...
            "run_object_alignment_from_json.py <task_meta.json or filename> [blender_path]"
        ),
    ...
    if OBJECT_ALIGNMENT_MODE == "foundationpose":
        best, final_camera_local_unity_debug = solve_foundationpose_alignment(
            task=task,
            paths=paths,
            model_vertices_unity=model_vertices_unity,
            target_front_fit=target_front_fit,
    ...
    elif OBJECT_ALIGNMENT_MODE == "camera_refine":
    ...
    elif OBJECT_ALIGNMENT_MODE == "off":
        ...
        raise ValueError(f"Unsupported OBJECT_ALIGNMENT_MODE: {OBJECT_ALIGNMENT_MODE}")
